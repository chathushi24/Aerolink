...
logger = logging.getLogger("payment-event-publisher")
...
def publish_event(event_type: str, detail: dict):
    logger.info(f"Publishing payment event {event_type} to EventBridge: {json.dumps(detail)}")
    ...
    try:
        ...
        logger.info(f"Successfully published payment event to EventBridge. Response: {json.dumps(response)}")
        return response
    except Exception as e:
        logger.error(f"EventBridge failed for payments: {str(e)}")
        raise e
